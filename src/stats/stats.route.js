...
router.get('/user-stats/:email', async (req,res) => {
    ...
    try {
        ...
        const totolPurchasedProducts = purchasedProductsIds.length;

        return successResponse(res,200,"Fetch user stats successfully",{
            totalPayments: totalpaymentsAmount.toFixed(2),
            totalReviews,
            totolPurchasedProducts
        })
        ...
        // console.log(totalPaymentsResult)

    } catch (error) {
        return errorResponse(res,500, "could not get user stats",error)
    }
})
